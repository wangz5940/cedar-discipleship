...
for env_name in \
  ...
  COMPOSE_PROJECT_NAME AGP_CONTAINER_PREFIX AGP_DATA_DIR AGP_LOG_DIR AGP_RESOURCE_ROOT \
  AGP_WEB_PORT AGP_MYSQL_PORT AGP_JWT_SECRET AGP_TOKEN_TTL AGP_REFRESH_TOKEN_TTL \
...
AGP_REFRESH_TOKEN_TTL="${AGP_REFRESH_TOKEN_TTL:-}"
GOPROXY="${GOPROXY:-}"
GOSUMDB="${GOSUMDB:-}"
GOPRIVATE="${GOPRIVATE:-}"
GONOSUMDB="${GONOSUMDB:-}"
...
mkdir -p \
  "$AGP_DATA_DIR/mysql" \
  "$AGP_RESOURCE_ROOT" \
  "$AGP_DATA_DIR/backups/mysql" \
  "$AGP_LOG_DIR/backend" \
  "$AGP_LOG_DIR/frontend" \
  "$AGP_LOG_DIR/mysql" \
  "$MIGRATION_REPORT_DIR"
if [ "$(id -u)" -eq 0 ]; then
  chown 999:999 "$AGP_LOG_DIR/mysql"
fi
trap cleanup EXIT
...
export AGP_WEB_PORT AGP_MYSQL_PORT AGP_JWT_SECRET AGP_TOKEN_TTL AGP_REFRESH_TOKEN_TTL BOOTSTRAP_SUPERADMIN_USERNAME BOOTSTRAP_SUPERADMIN_PASSWORD BOOTSTRAP_SUPERADMIN_DISPLAY_NAME
